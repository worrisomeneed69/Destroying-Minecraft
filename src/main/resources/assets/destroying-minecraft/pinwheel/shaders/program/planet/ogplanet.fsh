...
vec3 PLANET_POS = vec3(VeilCamera.CameraPosition.x, 2700, VeilCamera.CameraPosition.z) - PLANET_OFFSET;
const float ZOOM = 0.001;
const vec3 LIGHT_DIR = normalize(vec3(1, -0.1, 1));
...
float map(in vec3 p, int iterations) {
    ...
    vec3 coneRayPos = rayPos - (vec3(0, -1.4, 0) * 800);
//    coneRayPos.xy *= rot2D(180);
//    coneRayPos.yz *= rot2D(-55);

    float chunkNoise = fbm2(pos * 0.03, iterations);
    float cone = sdCappedCone(coneRayPos, 1033.333333, 66.6667, 1006.6667) + chunkNoise * 200;
...
void main() {
    ...
    if(depth >= 1.0){
        ...
        if(hit) {
            ...
            fragColor = mix(fragColor, mainTexture, 0);
        } else {
